...
export default {
  ...
  data() {
    return {
      ...
      keepTime: this.$route.query.keepTime
        ? JSON.parse(this.$route.query.keepTime)
        : null, // 保持时间（秒）【静态反馈训练专用】
      num: this.$route.query.num ? JSON.parse(this.$route.query.num) : 1, // 单组重复次数
      time: JSON.parse(this.$route.query.time), // 间隔时间（s）
      groupCount: JSON.parse(this.$route.query.groupCount), // 组数
      intervalTime: JSON.parse(this.$route.query.intervalTime), // 组间隔时间（秒）
