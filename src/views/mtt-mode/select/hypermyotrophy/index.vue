<!--
 * @Author      : Mr.bin
 * @Date        : 2022-07-20 17:01:49
 * @LastEditTime: 2022-07-26 10:02:27
 * @Description : 肌肥大训练
-->
<template>
  <div class="hypermyotrophy">
    <!-- 语音播放 -->
    ...
    <div class="left">
      ...
      <div class="introduce">
        <div class="item">
          训练要求：推荐使用70～80%的负荷，尽量贴合绿色曲线，锻炼下肢肌肥大
        </div>
        <div class="item">推荐组数：3~6组</div>
      </div>
      <!-- 示意图 -->
...
export default {
  ...
  data() {
    return {
      ...
      audioSrc: path.join(__static, `narrate/mandarin/13-肌肥大训练.mp3`),

      standardDistance: null, // 位移基准值，精确到1mm，有正负，用于调零

      num: 9, // 单组重复次数
      groupCount: 3, // 组数
      intervalTime: 60, // 组间隔时间（秒）
      bearWeightNum: 14, // 负重块（2.5kg/块）
      trainPart: '左腿', // 训练部位
      options: [
        {
          trainPart: '左腿'
    ...
    handleStart() {
      this.$router.push({
        ...
        query: {
          ...
          trainType: JSON.stringify('肌肥大训练'),
          routerName: JSON.stringify('/mtt-select/hypermyotrophy')
        }
      })
    },
